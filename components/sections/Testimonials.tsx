        ...
        <div style={{ display: 'grid', gap: 16 }} className="grid grid-cols-1 md:grid-cols-3">
          {tm.items.map((item, i) => (
            <motion.article key={item.name}
              ...
              style={{ background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid var(--border-subtle)', borderRadius: 16, padding: '32px 28px', display: 'flex', flexDirection: 'column', gap: 24 }}
            >
              <div style={{ color: 'var(--accent-gold)', fontSize: 13, letterSpacing: '0.05em' }} aria-label="5 stars">★★★★★</div>
              <blockquote style={{ fontSize: 15.5, lineHeight: 1.72, color: 'rgba(240,240,240,0.82)', flex: 1, fontFamily: '"DM Serif Display", serif', fontStyle: 'italic', margin: 0 }}>
